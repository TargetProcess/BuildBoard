...
trait ParseFolder extends ParseTestResults with FileHelper {
      ...
      getBuildDetails(folder)
        .map(buildDetails => {
          ...
          val maybeNode = watch(s"Get node ${buildSource.branch} ${buildSource.number}") {
            getBuildNode(buildSource.number, new Folder(buildSource.folder, "Build"), existingBuild)
          }
  ...
  def getBuildNode(buildNumber: Int, f: File, existingBuild: Option[Build]): Option[BuildNode] = {
    ...
    def getBuildNodeInner(existingBuildNode: Option[BuildNode], folder: File, path: String): Option[BuildNode] = {
      ...
      val maybeBuildNode = buildDetails
        .map(buildDetails => {
          val jobRun = getJobRun(buildNumber, name, buildDetails, artifacts)
          val status = calculateStatus(jobRun, buildDetails)
          updateJobRun(jobRun)
  ...
  private def calculateStatus(jobRun: JobRun, buildDetails: BuildDetails) : Option[String] = {
    if (jobRun.FailedTests == Nil)
      buildDetails.status
    else {
      val oldJobRuns =
        DefaultRegistry.jobRunRepository.getJobRuns(jobRun.buildNumber, jobRun.BuildNode)
          .toList.filter(oldJobRun => oldJobRun.id != jobRun.id)
      val jobRunsCount = oldJobRuns.length
      if (jobRunsCount > 0) {
        val filedTestExistsInAllRuns = jobRun.FailedTests.exists(filedTest => {
          oldJobRuns.forall(oldJobRun => oldJobRun.FailedTests.exists(test => test.name == filedTest.name))
        })

        if (filedTestExistsInAllRuns) Option("FAILURE") else Option("SUCCESS")
      }
      else
        buildDetails.status
    }
  ...
  private def updateJobRun(jobRun: JobRun) = {
    DefaultRegistry.jobRunRepository.update(jobRun)
  }

  private def getJobRun(buildNumber: Int, nodeName: String, buildDetails: BuildDetails, artifacts: List[Artifact]): JobRun = {
    val id = s"$buildNumber-${nodeName}-${buildDetails.startTime}"
    JobRun(id, buildNumber, nodeName, getFailedTestCases(artifacts), buildDetails.startTime, buildDetails.endTime)
  }
